...
#########################----END OF SIDEBAR------##########################
filelist=[]
for root, dirs, files in os.walk("prepared_csv_files"):
      for file in files:
             filename=os.path.join(root, file)
             filelist.append(filename)
st.write(filelist)

df_test1 = pd.read_csv("prepared_csv_files/test_trial1.csv")
df_test1
...
def get_data():
    if url_list:
        ...
        df = pd.read_csv(url_list[0]['filepath'])
...
if url_list:
    ...
    if url_list[0]['type_of_trial'] == "CMJ":
        ...
        for i in range (take_off_time, len(df.index)):
            if df.loc[i,'Force'] > 80:
                ...
                break
        # Find Start Try Time
        for i in range(1,take_off_time):
            
            if df.loc[i,'Force'] < (df['Force'].mean() - 80):# (df.loc[i-1,'Force'] * 0.25)): # df.loc[i-1,'Force'] - (df.loc[i-1,'Force'] * 0.002): #(df['Force'].mean() - 80):
                start_try_time = i
                break
    ...
    with st.form("Form for times",clear_on_submit = False):
        ...
        if url_list[0]['type_of_trial'] == "CMJ" or url_list[0]['type_of_trial'] == "SJ" or url_list[0]['type_of_trial'] == "DJ":
            ...
            with c8:
                user_time_input_rfd_till_time = st.number_input("RFD till time:", value=0, step=1,help="Time from end RFD")
    ...
    if brushed_submitted or st.session_state.load_state:
        ...
        if url_list[0]['type_of_trial'] == "ISO":
            ...
            # The whole RFD:
            X_all = df.loc[user_time_input_rfd_from_time:user_time_input_rfd_till_time, 'Rows_Count'] - df.loc[user_time_input_rfd_from_time:user_time_input_rfd_till_time, 'Rows_Count'].mean()
            Y_all = df.loc[user_time_input_rfd_from_time:user_time_input_rfd_till_time, 'Force'] - df.loc[user_time_input_rfd_from_time:user_time_input_rfd_till_time, 'Force'].mean()
            b_rfd1_whole = (X_all*Y_all).sum() / (X_all ** 2).sum()
            RFP_Total = pd.Series(b_rfd1_whole)
        ...
        if url_list[0]['type_of_trial'] == 'ISO':
            ...
            specific_metrics = {#'Unit': ['results'],
                    ...
                    'Force Mean (N)' : [df_brushed['Force'].mean()],
                    'Force Max (N)' : [max(df_brushed['Force'])],  
                    'RFD Total ' + str(user_time_input_rfd_from_time) + ' - ' + str(user_time_input_rfd_till_time) : [b_rfd1_whole]                
                    }
                ...
                def add_entries_to_jumps_statistics_table(supabase):
                        value = {'id': url_id_number_input, 'fullname': url_list[0]['fullname'], "age": url_list[0]['age'] ,\
                                ...
                                'landing_time': landing_time , 'rsi' : rsi, 'impulse_bw' : impulse_bw, 'impulse_grf':impulse_grf,
                                 'RFD Total': round(RFP_Total[0],4) if RFP_Total[0] is not None else RFP_Total[0],'date_of_trial' :url_list[0]['created_at']
                                }
